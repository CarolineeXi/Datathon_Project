...
```{r, cache=FALSE}
renderPlot({ 
  # If block to make the graph respond to user inputs
...
ggplot(data_1, aes(x=reorder(STUB_LABEL, EST), y = EST)) + 
  ...
  coord_flip() +
  ggtitle("Drug Overdose By Age and Sex")+
...
ggplot(data_2, aes(x=YEAR, y = AGE, fill = EST))+
  ...
  ggtitle("Drug Overdose Over Time By Age") +
  ylab("Age Group")+
  xlab("Year")+
  labs(fill = "Estimate \n (Deaths Per 100,000)")
})
...
ggplot(data_3, aes(x=reorder(RACE, ESTIMATE), y = ESTIMATE, fill = RACE)) + 
  geom_bar(stat = "identity") +
  ggtitle("Overdose Deaths By Race") +
  coord_flip() +
  labs(y = "Estimate (Deaths per 100,000 summed over the years selected)",
       x = "Race") + 
...
ggplot(data_4, aes(x=reorder(AGE, ESTIMATE), y = ESTIMATE, fill = AGE)) + 
  geom_bar(stat = "identity") +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  scale_fill_manual(values = c("#B2182B", "#D6604D", "#F4A582", "#FDDBC7", "#D1E5F0", "#92C5DE", "#4393C3", "#2166AC", "#053061"))
})
```
...
renderPlot({ 
  ...
  ggplot(data_5, aes(x=reorder(SEX, ESTIMATE), y = ESTIMATE, fill = SEX)) + 
  geom_bar(stat = "identity", width = 0.3) +
  ggtitle("Overdose Deaths By Sex") +
  labs(y="Estimate (Deaths per 100,000 summed over the years selected)",
       x = "Gender") +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black")) +
...
Method b:
* Created indexes vector containing indexes of rows with desired characteristic
* Mutated new column with indexes named after desired characteristic if index is in index vector
* Values without desired characteristic turned to NA
